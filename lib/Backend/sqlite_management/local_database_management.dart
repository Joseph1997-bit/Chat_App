...
class LocalDatabase {
  ...
  ///factory word For access Singleton object
  ...
  /// Insert or Update From Important Data Table and the data will be stored in the phone
  ...
  Future<String?> getUserNameForCurrentUser(String userEmail) async {
    try {
      final Database db = await this.database;
//bu sekilde kullanicinin adi verintabinindan alabiliriz
...
//bu fonksyonu kullanark belirli veya istedigmiz bilgiyi veritabanindan getirebilirz
  Future<String?> getParticularFieldDataFromImportantTable(
    ...
    try {
      ...
      _getFieldNameHelpWithEnumerators(getField);//bu fonksyona gelen parametre hangi alani/field alacagimizi belirliyo
  ...
  String? _getFieldNameHelpWithEnumerators(
    ...
    switch (getField) {//eger gelen deger userName isr onu dondur ve boylece asgada kosullar
  ...
  Future<void> createTableForEveryUser({required String userName}) async {
    try {
      final Database db = await this.database;
      //her kullanici icin bu sekilde tablo olusturabiliriz
      await db.execute(
...
//her kullanici icin tablo olusturduktan sonra bu sekilde kullanici bilgileri atip saklayabiliriz
  ...
  Future<List<PreviousMessageStructure>> getAllPreviousMessages(String userName) async {
    try {
      final Database db = await this.database;
